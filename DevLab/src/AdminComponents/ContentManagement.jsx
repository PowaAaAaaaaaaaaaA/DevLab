...
useEffect(() => {
  ...
  const handleDragEnd = async (event,lessonId, levelId) => {
    ...
    setLevelStages(prev => {
      ...
      const activeIndex = updatedStages.findIndex(stage => stage.id === active.id);
      const overIndex = updatedStages.findIndex(stage => stage.id === over.id);

      if (activeIndex === -1 || overIndex === -1) return prev;

      const reorderedStages = arrayMove(updatedStages, activeIndex, overIndex)
    ...
    <div className='h-full overflow-hidden'>
      ...
      <div className='border-b border-white h-auto flex flex-col justify-between p-5'>
        <div className='flex text-white font-exo justify-between p-10'>
          ...
          <button
            onClick={openPopup}
            className='rounded-2xl w-[15%] h-[60%] flex gap-5 items-center justify-center bg-[#4CAF50] font-bold hover:cursor-pointer hover:scale-105 transition duration-300 ease-in-out hover:drop-shadow-[0_0_6px_rgba(95,220,112,0.8)]'>
            <span className=" text-2xl"><HiArrowDownTray /></span>New Activity
          </button>
        </div>
      ...
      {isLoading ? (
        <Lottie animationData={Animation} loop={true} className="w-[60%] h-[70%] m-auto" />
      ) : (
            ...
            <div key={lesson.id} className="p-5 flex flex-col gap-15">
              ...
              <div className="flex flex-wrap justify-center gap-10">
                {lesson.levels.map((level) => (
                  <div key={level.id}
                    className="relative border-[#56EBFF] border w-[42%] p-5 flex flex-col gap-4 min-h-[180px] rounded-2xl bg-[#111827] transition-all duration-400 ">
                    ...
                    <div className="flex gap-5">
                      <div className="border mt-3 flex flex-wrap p-2 rounded-lg border-gray-500 gap-3 w-[100%]">
                        <DndContext collisionDetection={closestCorners} onDragEnd={(event) => handleDragEnd(event, lesson.Lesson, level.id)}>
                          <SortableContext
  items={levelStages[`${lesson.Lesson}_${level.id}`]?.map(stage => stage.id) || []}
  strategy={horizontalListSortingStrategy}
>
  {levelStages[`${lesson.Lesson}_${level.id}`]?.map(stage => (
    <SortableStage
      key={stage.id}
      stage={stage}
      onClick={() => {
    ...
    />
                    ...
                    <div className="flex justify-end gap-3 absolute top-3 right-5">
                      <div className="text-white text-2xl">
                        <button className="hover:cursor-pointer hover:bg-gray-600 rounded p-2 border-gray-500 border"
                          onClick={() => addNewTopicMutation.mutate({ subject: activeTab, lessonId: lesson.Lesson, levelId: level.id })}>
                          <GoPlus />
                        </button>
                      </div>
                      <div className="text-white text-2xl">
                        <button className="hover:cursor-pointer hover:bg-gray-600 rounded p-2 border-gray-500 border"
                          onClick={() => DeleteLevelMutaion.mutate({ subject: activeTab, lessonId: lesson.Lesson, levelId: level.id })}>
                          <GoTrash />
                        </button>
                      </div>
                    </div>
